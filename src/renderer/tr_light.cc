...
void R_TransformDlights( int count, dlight_t *dl, orientationr_t *orx ) {
	...
	for ( i = 0 ; i < count ; i++, dl++ ) {
        VectorSubtract( dl->origin, orx->origin, temp );
        dl->transformed[0] = DotProduct( temp, orx->axis[0] );
        dl->transformed[1] = DotProduct( temp, orx->axis[1] );
        dl->transformed[2] = DotProduct( temp, orx->axis[2] );
	}
}
...
void R_DlightBmodel( bmodel_t *bmodel ) {
    ...
    R_TransformDlights( tr.refdef.num_dlights, tr.refdef.dlights, &tr.orx );
