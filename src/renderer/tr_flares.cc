...
typedef struct flare_s {
	...
	bool cgvisible;             // for coronas, the client determines current visibility, but it's still inserted so it will fade out properly
...
==================
*/
void RB_AddFlare( void *surface, int fogNum, vec3_t point, vec3_t color, float scale, vec3_t normal, int id, bool cgvisible ) { //----(SA)	added scale. added id.  added visible
	int i;
	flare_t         *f, *oldest;
	vec3_t local;
	float d;
    ...
    R_TransformModelToClip( point, backEnd.orx.modelMatrix,
...
//		if ( f->surface == surface && f->frameSceneNum == backEnd.viewParms.frameSceneNum && f->inPortal == backEnd.viewParms.isPortal ) {
	...
	if ( normal ) {
        VectorSubtract( backEnd.viewParms.orx.origin, point, local );
