...
static void FillCloudySkySide( const int mins[2], const int maxs[2], bool addIndexes ) {
	...
	for ( t = mins[1] + HALF_SKY_SUBDIVISIONS; t <= maxs[1] + HALF_SKY_SUBDIVISIONS; t++ )
	{
		for ( s = mins[0] + HALF_SKY_SUBDIVISIONS; s <= maxs[0] + HALF_SKY_SUBDIVISIONS; s++ )
		{
            VectorAdd( s_skyPoints[t][s], backEnd.viewParms.orx.origin, tess.xyz[tess.numVertexes] );
...
void RB_DrawSun( void ) {
    ...
    qglTranslatef( backEnd.viewParms.orx.origin[0], backEnd.viewParms.orx.origin[1], backEnd.viewParms.orx.origin[2] );
...
*/
	...
	if ( r_drawSun->integer > 1 ) { // draw flare effect
        ...
        VectorAdd( tr.sunDirection, backEnd.viewParms.orx.axis[0], temp );
