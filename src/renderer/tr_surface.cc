...
void RB_AddQuadStampFadingCornersExt( vec3_t origin, vec3_t left, vec3_t up, byte *color, float s1, float t1, float s2, float t2 ) {
    ...
    VectorSubtract( vec3_origin, backEnd.viewParms.orx.axis[0], normal );
...
static void RB_SurfaceSprite( void ) {
	...
	if ( backEnd.currentEntity->e.rotation == 0 ) {
        VectorScale( backEnd.viewParms.orx.axis[1], radius, left );
        VectorScale( backEnd.viewParms.orx.axis[2], radius, up );
	} else {
		float s, c;
		float ang;
        ...
        VectorScale( backEnd.viewParms.orx.axis[1], c * radius, left );
        VectorMA( left, -s * radius, backEnd.viewParms.orx.axis[2], left );

        VectorScale( backEnd.viewParms.orx.axis[2], c * radius, up );
        VectorMA( up, s * radius, backEnd.viewParms.orx.axis[1], up );
	}
	if ( backEnd.viewParms.isMirror ) {
		VectorSubtract( vec3_origin, left, left );
	}
...
void RB_SurfaceRailCore( void ) {
	...
	// compute side vector
    VectorSubtract( start, backEnd.viewParms.orx.origin, v1 );
	VectorNormalize( v1 );
    VectorSubtract( end, backEnd.viewParms.orx.origin, v2 );
	VectorNormalize( v2 );
...
static float    LodErrorForVolume( vec3_t local, float radius ) {
    ...
    world[0] = local[0] * backEnd.orx.axis[0][0] + local[1] * backEnd.orx.axis[1][0] +
               local[2] * backEnd.orx.axis[2][0] + backEnd.orx.origin[0];
    world[1] = local[0] * backEnd.orx.axis[0][1] + local[1] * backEnd.orx.axis[1][1] +
               local[2] * backEnd.orx.axis[2][1] + backEnd.orx.origin[1];
    world[2] = local[0] * backEnd.orx.axis[0][2] + local[1] * backEnd.orx.axis[1][2] +
               local[2] * backEnd.orx.axis[2][2] + backEnd.orx.origin[2];

    VectorSubtract( world, backEnd.viewParms.orx.origin, world );
    d = DotProduct( world, backEnd.viewParms.orx.axis[0] );
...
void RB_SurfaceFlare( srfFlare_t *surf ) {
    ...
    VectorSubtract( origin, backEnd.viewParms.orx.origin, dir );
	VectorNormalize( dir );
