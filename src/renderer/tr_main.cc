...
int R_CullLocalBox( vec3_t bounds[2] ) {
	...
	for ( i = 0 ; i < 8 ; i++ ) {
        ...
        VectorCopy( tr.orx.origin, transformed[i] );
        VectorMA( transformed[i], v[0], tr.orx.axis[0], transformed[i] );
        VectorMA( transformed[i], v[1], tr.orx.axis[1], transformed[i] );
        VectorMA( transformed[i], v[2], tr.orx.axis[2], transformed[i] );
	}
...
void R_LocalNormalToWorld( vec3_t local, vec3_t world ) {
    world[0] = local[0] * tr.orx.axis[0][0] + local[1] * tr.orx.axis[1][0] + local[2] * tr.orx.axis[2][0];
    world[1] = local[0] * tr.orx.axis[0][1] + local[1] * tr.orx.axis[1][1] + local[2] * tr.orx.axis[2][1];
    world[2] = local[0] * tr.orx.axis[0][2] + local[1] * tr.orx.axis[1][2] + local[2] * tr.orx.axis[2][2];
}
...
void R_LocalPointToWorld( vec3_t local, vec3_t world ) {
    world[0] = local[0] * tr.orx.axis[0][0] + local[1] * tr.orx.axis[1][0] + local[2] * tr.orx.axis[2][0] + tr.orx.origin[0];
    world[1] = local[0] * tr.orx.axis[0][1] + local[1] * tr.orx.axis[1][1] + local[2] * tr.orx.axis[2][1] + tr.orx.origin[1];
    world[2] = local[0] * tr.orx.axis[0][2] + local[1] * tr.orx.axis[1][2] + local[2] * tr.orx.axis[2][2] + tr.orx.origin[2];
}
...
void R_WorldToLocal( vec3_t world, vec3_t local ) {
    local[0] = DotProduct( world, tr.orx.axis[0] );
    local[1] = DotProduct( world, tr.orx.axis[1] );
    local[2] = DotProduct( world, tr.orx.axis[2] );
}
...
void R_RotateForEntity( const trRefEntity_t *ent, const viewParms_t *viewParms,
    ...
    VectorCopy( ent->e.origin, orx->origin );

    VectorCopy( ent->e.axis[0], orx->axis[0] );
    VectorCopy( ent->e.axis[1], orx->axis[1] );
    VectorCopy( ent->e.axis[2], orx->axis[2] );

    glMatrix[0] = orx->axis[0][0];
    glMatrix[4] = orx->axis[1][0];
    glMatrix[8] = orx->axis[2][0];
    glMatrix[12] = orx->origin[0];

    glMatrix[1] = orx->axis[0][1];
    glMatrix[5] = orx->axis[1][1];
    glMatrix[9] = orx->axis[2][1];
    glMatrix[13] = orx->origin[1];

    glMatrix[2] = orx->axis[0][2];
    glMatrix[6] = orx->axis[1][2];
    glMatrix[10] = orx->axis[2][2];
    glMatrix[14] = orx->origin[2];
    ...
    myGlMultMatrix( glMatrix, viewParms->world.modelMatrix, orx->modelMatrix );

	// calculate the viewer origin in the model's space
	// needed for fog, specular, and environment mapping
    VectorSubtract( viewParms->orx.origin, orx->origin, delta );
    ...
    orx->viewOrigin[0] = DotProduct( delta, orx->axis[0] ) * axisLength;
    orx->viewOrigin[1] = DotProduct( delta, orx->axis[1] ) * axisLength;
    orx->viewOrigin[2] = DotProduct( delta, orx->axis[2] ) * axisLength;
}
...
void R_RotateForViewer( void ) {
    ...
    memset( &tr.orx, 0, sizeof( tr.orx ) );
    tr.orx.axis[0][0] = 1;
    tr.orx.axis[1][1] = 1;
    tr.orx.axis[2][2] = 1;
    VectorCopy( tr.viewParms.orx.origin, tr.orx.viewOrigin );

	// transform by the camera placement
    VectorCopy( tr.viewParms.orx.origin, origin );

    viewerMatrix[0] = tr.viewParms.orx.axis[0][0];
    viewerMatrix[4] = tr.viewParms.orx.axis[0][1];
    viewerMatrix[8] = tr.viewParms.orx.axis[0][2];
	viewerMatrix[12] = -origin[0] * viewerMatrix[0] + - origin[1] * viewerMatrix[4] + - origin[2] * viewerMatrix[8];

    viewerMatrix[1] = tr.viewParms.orx.axis[1][0];
    viewerMatrix[5] = tr.viewParms.orx.axis[1][1];
    viewerMatrix[9] = tr.viewParms.orx.axis[1][2];
	viewerMatrix[13] = -origin[0] * viewerMatrix[1] + - origin[1] * viewerMatrix[5] + - origin[2] * viewerMatrix[9];

    viewerMatrix[2] = tr.viewParms.orx.axis[2][0];
    viewerMatrix[6] = tr.viewParms.orx.axis[2][1];
    viewerMatrix[10] = tr.viewParms.orx.axis[2][2];
	...
	// convert from our coordinate system (looking down X)
	// to OpenGL's coordinate system (looking down -Z)
    myGlMultMatrix( viewerMatrix, s_flipMatrix, tr.orx.modelMatrix );

    tr.viewParms.world = tr.orx;
...
static void SetFarClip( void ) {
	...
	for ( i = 0; i < 8; i++ )
	{
        ...
        VectorSubtract( v, tr.viewParms.orx.origin, vecTo );
...
void R_SetupFrustum( void ) {
    ...
    VectorScale( tr.viewParms.orx.axis[0], xs, tr.viewParms.frustum[0].normal );
    VectorMA( tr.viewParms.frustum[0].normal, xc, tr.viewParms.orx.axis[1], tr.viewParms.frustum[0].normal );

    VectorScale( tr.viewParms.orx.axis[0], xs, tr.viewParms.frustum[1].normal );
    VectorMA( tr.viewParms.frustum[1].normal, -xc, tr.viewParms.orx.axis[1], tr.viewParms.frustum[1].normal );

	ang = tr.viewParms.fovY / 180 * M_PI * 0.5f;
	xs = sin( ang );
	xc = cos( ang );

    VectorScale( tr.viewParms.orx.axis[0], xs, tr.viewParms.frustum[2].normal );
    VectorMA( tr.viewParms.frustum[2].normal, xc, tr.viewParms.orx.axis[2], tr.viewParms.frustum[2].normal );

    VectorScale( tr.viewParms.orx.axis[0], xs, tr.viewParms.frustum[3].normal );
    VectorMA( tr.viewParms.frustum[3].normal, -xc, tr.viewParms.orx.axis[2], tr.viewParms.frustum[3].normal );

	for ( i = 0 ; i < 4 ; i++ ) {
		tr.viewParms.frustum[i].type = PLANE_NON_AXIAL;
        tr.viewParms.frustum[i].dist = DotProduct( tr.viewParms.orx.origin, tr.viewParms.frustum[i].normal );
...
bool R_GetPortalOrientations( drawSurf_t *drawSurf, int entityNum,
	...
	if ( entityNum != ENTITYNUM_WORLD ) {
        ...
        R_RotateForEntity( tr.currentEntity, &tr.viewParms, &tr.orx );

		// rotate the plane, but keep the non-rotated version for matching
		// against the portalSurface entities
		R_LocalNormalToWorld( originalPlane.normal, plane.normal );
        plane.dist = originalPlane.dist + DotProduct( plane.normal, tr.orx.origin );

		// translate the original plane
        originalPlane.dist = originalPlane.dist + DotProduct( originalPlane.normal, tr.orx.origin );
	} else {
		plane = originalPlane;
	}
...
static bool SurfIsOffscreen( const drawSurf_t *drawSurf, vec4_t clipDest[128] ) {
	...
	for ( i = 0; i < tess.numVertexes; i++ )
	{
        ...
        R_TransformModelToClip( tess.xyz[i], tr.orx.modelMatrix, tr.viewParms.projectionMatrix, eye, clip );
	...
	for ( i = 0; i < tess.numIndexes; i += 3 )
	{
        ...
        VectorSubtract( tess.xyz[tess.indexes[i]], tr.viewParms.orx.origin, normal );
...
//	if ( r_noportals->integer || r_fastsky->integer || tr.levelGLFog) {
    ...
    R_MirrorPoint( oldParms.orx.origin, &surface, &camera, newParms.orx.origin );

	VectorSubtract( vec3_origin, camera.axis[0], newParms.portalPlane.normal );
	newParms.portalPlane.dist = DotProduct( camera.origin, newParms.portalPlane.normal );

    R_MirrorVector( oldParms.orx.axis[0], &surface, &camera, newParms.orx.axis[0] );
    R_MirrorVector( oldParms.orx.axis[1], &surface, &camera, newParms.orx.axis[1] );
    R_MirrorVector( oldParms.orx.axis[2], &surface, &camera, newParms.orx.axis[2] );
...
void R_AddEntitySurfaces( void ) {
	...
	for ( tr.currentEntityNum = 0;
		...
		case RT_MODEL:
            ...
            R_RotateForEntity( ent, &tr.viewParms, &tr.orx );
