...
int PC_EvaluateTokens( source_t *source, token_t *tokens, signed long int *intvalue,
	...
	while ( !error && firstoperator )
	{
		...
		for ( o = firstoperator; o->next; o = o->next )
		{
			...
			if ( o->operatorval != P_LOGIC_NOT
				 && o->operatorval != P_BIN_NOT ) {
				v = v->next;
			}
...
#ifdef DEBUG_EVAL
		if ( integer ) {
			Log_Write( "operator %s, value1 = %d", PunctuationFromNum( source->scriptstack, o->operatorval ), v1->intvalue );
			if ( v2 ) {
		...
		{
			Log_Write( "operator %s, value1 = %f", PunctuationFromNum( source->scriptstack, o->operatorval ), v1->floatvalue );
			if ( v2 ) {
				Log_Write( "value2 = %f", v2->floatvalue );
			}
		} //end else
#endif //DEBUG_EVAL
		switch ( o->operatorval )
		{
		...
		lastoperatortype = o->operatorval;
		//if not an operator with arity 1
		if ( o->operatorval !=P_LOGIC_NOT
			 && o->operatorval !=P_BIN_NOT ) {
			//remove the second value if not question mark operator
			if ( o->operatorval != P_QUESTIONMARK ) {v = v->next;}
			//
			if ( v->prev ) {
				v->prev->next = v->next;
