...
void clamp_playerbehindgun( gentity_t *self, gentity_t *other, vec3_t dang ) {
	...
	// save results of pmove
	BG_PlayerStateToEntityState( &other->client->ps, &other->s, false );
...
void miscGunnerEnemyScan( gentity_t *ent, vec3_t angles ) {
	...
	for ( t = g_entities; t < g_entities + level.maxclients; t++ ) {
		...
		if ( !AICast_VisibleFromPos( ent->r.currentOrigin, ent->s.number, t->r.currentOrigin, t->s.number, false ) ) {
			continue;
		}
		// found an enemy
		ent->enemy = t;
...
void miscGunnerThink( gentity_t *ent ) {
	...
	if ( ent->enemy ) {
		if ( ent->enemy->health <= 0 ) {
			ent->enemy = NULL;
		} else if ( AICast_VisibleFromPos( ent->r.currentOrigin, ent->s.number, ent->enemy->r.currentOrigin, ent->enemy->s.number, false ) ) {
			fire = true;
		}
	}
