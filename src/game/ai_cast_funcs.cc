	...
	if ( numEnemies == -1 ) {     // query mode
		...
		if ( cs->bs->enemy >= 0 ) {
			if ( ( ( cs->leaderNum < 0 ) || ( cs->thinkFuncChangeTime < level.time - 3000 ) ) && AICast_WantsToChase( cs ) ) {  // don't leave our leader as soon as we get to them
				return AIFunc_BattleStart( cs );
			} else if ( AICast_EntityVisible( AICast_GetCastState( cs->bs->enemy ), cs->entityNum, true ) || AICast_CheckAttack( AICast_GetCastState( cs->bs->enemy ), cs->entityNum, false ) ) {
...
*/                                                                                                                                                                                                                                                                                                                                    // if we only just recently saw them, face that direction
		if ( ( g_entities[cs->lastEnemy].health > 0 ) && cs->vislist[cs->lastEnemy].visible_timestamp > ( level.time - 20000 )
			 && AICast_VisibleFromPos( cs->bs->origin, cs->entityNum, cs->vislist[cs->lastEnemy].visible_pos, cs->lastEnemy, false ) ) {
			vec3_t dir;
			//
...
*/
	...
	if ( cs->nextIdleAngleChange < level.time ) {
		...
		if ( ( cs->nextIdleAngleChange + 3000 ) < level.time ) {
			...
			if ( (int)fabs( AngleDelta( cs->idleYaw, cs->bs->ideal_viewangles[YAW] ) ) < 45 ) {
				cs->nextIdleAngleChange = level.time + 1000 + rand() % 2500;
			} else { // do really fast
				cs->nextIdleAngleChange = level.time + 500;
			}
...
char *AIFunc_BattleDiveStart( cast_state_t *cs, vec3_t vec ) {
	...
	vectoangles( vec, cs->bs->ideal_viewangles );
	// do the roll
	duration = BG_AnimScriptEvent( &g_entities[cs->entityNum].client->ps, ANIM_ET_DIVE, false, false );
	//
	if ( duration < 0 ) { // it failed
		return NULL;
	}
...
char *AIFunc_BattleHunt( cast_state_t *cs ) {
	...
	if (    AICast_EntityVisible( cs, bs->enemy, true )    // take into account reaction time
			&&  AICast_CheckAttack( cs, bs->enemy, false )
...
char *AIFunc_BattleAmbush( cast_state_t *cs ) {
	...
	if ( VectorLength( cs->takeCoverPos ) > 1 && dist > 8 && ( cs->obstructingTime < level.time ) && !shouldAttack ) {
		...
		if ( !( cs->aiFlags & AIFL_MISCFLAG1 ) ) {
			if ( move.numtouch || !AICast_VisibleFromPos( move.endpos, cs->entityNum, cs->combatGoalOrigin, cs->bs->enemy, false ) ) {
	...
	if ( AICast_VisibleFromPos( cs->bs->origin, cs->entityNum, cs->combatGoalOrigin, cs->lastEnemy, false ) ) {
...
char *AIFunc_BattleChase( cast_state_t *cs ) {
	...
	if ( AICast_EntityVisible( cs, cs->bs->enemy, true ) && VectorDistance( cs->bs->origin, destorg ) < 384 ) {
	...
	if ( numEnemies > 0 ) {
		int i;
		for ( i = 0; i < numEnemies; i++ ) {
			if ( enemies[i] != cs->bs->enemy && AICast_CheckAttack( cs, enemies[i], false ) ) {
				cs->bs->enemy = enemies[i];
				return AIFunc_BattleStart( cs );
			}
		}
	...
	if (    !moved && cs->leaderNum < 0 &&
			( cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE || g_entities[cs->entityNum].waterlevel > 1 ) &&
			AICast_EntityVisible( cs, cs->bs->enemy, true ) ) {
		aicast_predictmove_t move;
		vec3_t dir;
		bot_input_t bi;
		usercmd_t ucmd;
	...
	// if we are going to reach them soon, predict the attack
	{
		...
		// do we went to play a diving animation into a cover position?
					...
					( AICast_CheckAttackAtPos( cs->entityNum, cs->bs->enemy, move.endpos, cs->bs->attackcrouch_time > trap_AAS_Time(), false ) ) ) {
		...
		// slow down? so we don't go too far from behind the obstruction which is protecting us
				  ...
				  ( AICast_VisibleFromPos( cs->vislist[cs->bs->enemy].visible_pos, cs->bs->enemy, move.endpos, cs->entityNum, false ) ) ) {
			// start a crouch attack?
			//if (cs->attributes[ATTACK_CROUCH] > 0.1) {
			//	cs->bs->attackcrouch_time = trap_AAS_Time() + 3.0;
			//else
...
char *AIFunc_AvoidDanger( cast_state_t *cs ) {
	...
	if ( dist > 8 ) {
		...
		if ( ent->s.onFireEnd < level.time && cs->bs->enemy >= 0 && cs->vislist[cs->bs->enemy].real_visible_timestamp && ( cs->vislist[cs->bs->enemy].real_visible_timestamp > level.time - 10000 ) ) {
			AICast_UpdateVisibility( &g_entities[cs->entityNum], &g_entities[cs->bs->enemy], false, cs->vislist[cs->bs->enemy].real_visible_timestamp == cs->vislist[cs->bs->enemy].lastcheck_timestamp );
		}
...
char *AIFunc_BattleTakeCover( cast_state_t *cs ) {
	...
	if ( !shouldAttack ) {
		...
		if ( AICast_VisibleFromPos( g_entities[bs->enemy].client->ps.origin, bs->enemy, cs->takeCoverPos, bs->entitynum, false ) ) {
			...
			if ( AICast_EntityVisible( AICast_GetCastState( cs->bs->enemy ), cs->entityNum, true ) || AICast_CheckAttack( AICast_GetCastState( cs->bs->enemy ), cs->entityNum, false ) ) {
				if ( !AICast_GetTakeCoverPos( cs, bs->enemy, cs->vislist[bs->enemy].visible_pos, cs->takeCoverPos ) ) {
					// shit!! umm.. try and fire?
					return AIFunc_BattleStart( cs );
				} else {    // recalc distance
...
// DISABLED: doesn't work well with new AI system
	...
	if ( VectorLength( cs->takeCoverPos ) > 1 && dist > 8 ) {
		...
		// do we went to play a rolling animation into a cover position?
					...
					( shouldAttack && !AICast_VisibleFromPos( g_entities[cs->bs->enemy].s.pos.trBase, cs->bs->enemy, move.endpos, cs->entityNum, false ) ) ) {
		...
		if ( !( cs->aiFlags & AIFL_MISCFLAG1 ) && !AICast_VisibleFromPos( cs->vislist[cs->bs->enemy].real_visible_pos, cs->bs->enemy, move.endpos, cs->entityNum, false )
			 && !AICast_VisibleFromPos( cs->vislist[cs->bs->enemy].real_visible_pos, cs->bs->enemy, cs->bs->origin, cs->entityNum, false ) ) {
	...
	if ( shouldAttack ) {
		...
		if ( ( cs->thinkFuncChangeTime < level.time - 1000 ) && ( AICast_VisibleFromPos( g_entities[bs->enemy].client->ps.origin, bs->enemy, cs->takeCoverPos, bs->entitynum, false ) ) ) {
			// abort
			return AIFunc_BattleStart( cs );
		}
		//
...
char *AIFunc_GrenadeFlush( cast_state_t *cs ) {
	...
	if (    AICast_CheckAttack( cs, bs->enemy, false )
	...
	if ( !AICast_EntityVisible( cs, cs->bs->enemy, true ) ) {
...
char *AIFunc_BattleMG42( cast_state_t *cs ) {
	...
	if (    bs->enemy < 0 ||
		...
		if ( !shouldAttack ) {
			// keep firing at anything behind solids, in case they find a position where they can shoot us, but our checkattack() doesn't find a clear shot
			if ( bs->enemy < 0 || !AICast_CheckAttack( cs, bs->enemy, true ) ||
...
char *AIFunc_Battle( cast_state_t *cs ) {
	...
	if (    ( cs->bs->cur_ps.weaponTime < 100 )   // if reloading, don't chase until ready
			...
			&&  ( !AICast_EntityVisible( cs, bs->enemy, true ) || !AICast_CheckAttack( cs, bs->enemy, false ) ) ) {
	...
	// if the enemy is really close, avoid them
				...
				( Distance( cs->bs->origin, cs->vislist[cs->bs->enemy].real_visible_pos ) < 100 ) ) {
		if ( AICast_GetAvoid( cs, NULL, cs->obstructingPos, true, cs->bs->enemy ) ) {
	...
	if (    ( cs->attributes[AGGRESSION] < 1.0 ) &&
		...
		if ( DotProduct( aim, enemyVec ) > 0.97 ) {
			...
			if ( move.groundEntityNum == ENTITYNUM_WORLD &&
				...
				// good enough
				if ( AICast_CheckAttackAtPos( cs->entityNum, cs->bs->enemy, move.endpos, cs->bs->cur_ps.viewheight == cs->bs->cur_ps.crouchViewHeight, false ) ) {
...
recheck:
	...
	if ( fabs( AngleDifference( cs->bs->ideal_viewangles[YAW], cs->bs->viewangles[YAW] ) ) < 10 ) {
		...
		if ( ( cs->bs->weaponnum == WP_MONSTER_ATTACK1 ) && cs->aifuncAttack1 ) {
			if ( AICast_CheckAttack( cs, cs->bs->enemy, false ) ) {
