...
================
*/
void limbo( gentity_t *ent, bool makeCorpse ) {
	int i,contents;
...
//----(SA) added this for head separation
	...
	if ( atoi( s ) ) {
		client->pers.bAutoReloadAux = true;
		client->pmext.bAutoReload = true;
	} else {
		client->pers.bAutoReloadAux = false;
		client->pmext.bAutoReload = false;
	}
...
char *ClientConnect( int clientNum, bool firstTime, bool isBot ) {
	...
	if ( firstTime ) {
		client->pers.initialSpawn = true;              // DHM - Nerve
...
void ClientSpawn( gentity_t *ent, bool revived ) {
	...
	int savedPing;
	int savedTeam;
	bool savedVoted = false;         // NERVE - SMF
