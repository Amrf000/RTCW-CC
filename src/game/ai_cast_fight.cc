...
void AICast_UpdateBattleInventory( cast_state_t *cs, int enemy ) {
	...
	for ( i = 0; i < MAX_WEAPONS; i++ ) {
		if ( g_entities[cs->bs->entitynum].client->ps.ammo[ BG_FindAmmoForWeapon((weapon_t)i )] > 800 ) {
			...
			Add_Ammo( &g_entities[cs->entityNum], cs->bs->weaponnum, 999, false );
		}
	}
...
bool AICast_WeaponUsable( cast_state_t *cs, int weaponNum ) {
	...
	case WP_ROCKET_LAUNCHER:
		...
		case AICHAR_STIMSOLDIER2:
			return false;  // stime only uses Rocket Launcher in jumping attack
	...
	if ( cs->combatGoalTime > level.time ) {
		if ( VectorLength( cs->combatGoalOrigin ) > 1 ) {
			...
			if ( ( dist = Distance( goal.origin, cs->bs->origin ) ) < 32 ) {
				...
				//
				if ( move.numtouch || !AICast_CheckAttackAtPos( cs->entityNum, cs->bs->enemy, move.endpos, false, false ) ) {
		...
		} else if ( !AICast_CheckAttack( cs, cs->bs->enemy, false ) &&
					AICast_VisibleFromPos( cs->vislist[cs->bs->enemy].visible_pos, cs->bs->enemy, cs->bs->origin, cs->entityNum, false ) ) {
			cs->combatGoalTime = 0;
			cs->combatSpotAttackCount = cs->startAttackCount;
		}
...
void AICast_ProcessAttack( cast_state_t *cs ) {
	...
	if ( !AICast_CheckAttack( cs, bs->enemy, false ) ) {
		...
		if ( cs->attributes[ATTACK_CROUCH] && ( cs->castScriptStatus.scriptNoMoveTime >= level.time ) ) {
			if ( !AICast_CheckAttackAtPos( cs->entityNum, bs->enemy, cs->bs->origin, false, false ) ) {
