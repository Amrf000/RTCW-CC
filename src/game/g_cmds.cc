...
void SetTeam( gentity_t *ent, char *s ) {
	...
	if ( client->pers.initialSpawn && team != TEAM_SPECTATOR ) {
		client->pers.initialSpawn = false;
	}
...
void G_SayTo( gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message, bool localize ) { // removed static so it would link
	if ( !other ) {
		return;
	}
	if ( !other->inuse ) {
...
void Cmd_Activate_f( gentity_t *ent ) {
	...
	if ( traceEnt->classname ) {
		...
		} else if ( ( Q_stricmp( traceEnt->classname, "misc_mg42" ) == 0 ) && traceEnt->active == false )         {
			if (
...
//		Cmd_Say_f (ent, false, true);			// NERVE - SMF - we don't want to spam the clients with this.
		return;
	}
