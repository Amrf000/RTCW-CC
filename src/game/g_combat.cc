...
==================
*/
void limbo( gentity_t *ent, bool makeCorpse ); // JPW NERVE
...
// jpw
		...
		self->client->ps.pm_time = BG_AnimScriptEvent( &self->client->ps, ANIM_ET_DEATH, false, true );
...
bool IsHeadShotWeapon( int mod, bool aicharacter ) {
...
bool IsHeadShot( gentity_t *targ, bool isAICharacter, vec3_t dir, vec3_t point, int mod ) {
	gentity_t   *head;
	trace_t tr;
	vec3_t start, end;
	gentity_t   *traceEnt;
...
// jpw
	...
	if ( IsHeadShot( targ, false, dir, point, mod ) ) {
