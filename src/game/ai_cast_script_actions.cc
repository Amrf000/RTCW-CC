			...
			} else
			{
				...
				if ( !token[0] || !Q_stricmp( token,"nostop" ) ) {
					...
					if ( !token[0] || Q_stricmp( token,"noattack" ) ) {
						...
						if ( fire ) {
							for ( i = 0; i < 2; i++ ) {
								diff = (int)fabs( AngleDifference( cs->bs->viewangles[i], cs->bs->ideal_viewangles[i] ) );
								if ( diff < 20 ) {
									// force fire
		...
		if ( moverange > 0 ) {
			...
			if (    ( cs->castScriptStatus.scriptWaitMovetime < level.time )
				...
				if (    AICast_CheckAttack( cs, cs->bs->enemy, false )
						||  (   !AICast_EntityVisible( AICast_GetCastState( cs->bs->enemy ), cs->entityNum, false )
								&&  !AICast_CheckAttack( AICast_GetCastState( cs->bs->enemy ), cs->entityNum, false ) ) ) {
...
bool AICast_ScriptAction_PlayAnim( cast_state_t *cs, char *params ) {
	...
	if ( level.animScriptData.modelInfo[level.animScriptData.clientModels[cs->entityNum] - 1].version > 1 ) { // new (scripted) model
		...
		if ( cs->castScriptStatus.scriptFlags & SFL_FIRST_CALL ) {
			...
			BG_PlayAnimName( &( client->ps ), tokens[0], (animBodyPart_t)BG_IndexForString( tokens[1], (animStringItem_t*)animBodyPartsStr, false ), true, false, true );
...
//----(SA)	// trying this with Add_Ammo() again so automatic stuff happens automatically
		Add_Ammo( &g_entities[cs->entityNum], weapon, atoi( token ), true );
...
=================
*/
bool AICast_ScriptAction_MissionSuccess( cast_state_t *cs, char *params ) {
	gentity_t *player;
