...
int CG_DrawField( int x, int y, int width, int value, int charWidth, int charHeight, bool dodrawpic, bool leftAlign ) {
	char num[16], *ptr;
	int l;
	int frame;
	int startx;
...
static void CG_DrawPickupItem( void ) {
	...
	if ( value ) {
		...
		if ( fadeColor ) {
			...
			color[3] = fadeColor[0];
			CG_DrawStringExt2( 34, 388, s, color, false, true, 10, 10, 0 ); // JPW NERVE moved per atvi req
	...
	if ( cgs.notifyLastPos != cgs.notifyPos ) {
		...
		for ( i = cgs.notifyPos - 1; i >= cgs.notifyLastPos; i-- ) {
			...
			CG_DrawStringExt( NOTIFYLOC_X + TINYCHAR_WIDTH,
							  ...
							  cgs.notifyMsgs[i % chatHeight], hcolor, false, false,
...
void CG_CenterPrint( const char *str, int y, int charWidth ) {
	char    *s;
	int i, len;                         // NERVE - SMF
	bool neednewline = false;      // NERVE - SMF
	int priority = 0;
...
static void CG_DrawWeapReticle( void ) {
	bool snooper, sniper, fg;
	vec4_t color = {0, 0, 0, 1};

	// DHM - Nerve :: So that we will draw reticle
	if ( cgs.gametype >= GT_WOLF && ( ( cg.snap->ps.pm_flags & PMF_FOLLOW ) || cg.demoPlayback ) ) {
		sniper = (bool)( cg.snap->ps.weapon == WP_SNIPERRIFLE );
		snooper = (bool)( cg.snap->ps.weapon == WP_SNOOPERSCOPE );
		fg = (bool)( cg.snap->ps.weapon == WP_FG42SCOPE );
	} else {
		sniper = (bool)( cg.weaponSelect == WP_SNIPERRIFLE );
		snooper = (bool)( cg.weaponSelect == WP_SNOOPERSCOPE );
		fg = (bool)( cg.weaponSelect == WP_FG42SCOPE );
	}
...
static void CG_DrawVote( void ) {
	...
	if ( cgs.complaintEndTime > cg.time ) {
		...
		if ( cgs.complaintClient == -1 ) {
			...
			CG_DrawStringExt( 8, 200, CG_TranslateString( s ), color, true, false, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 80 );
			return;
		}
		...
		CG_DrawStringExt( 8, 200, s, color, true, false, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 80 );

		s = va( CG_TranslateString( "Press '%s' for YES, or '%s' for No" ), str1, str2 );
		CG_DrawStringExt( 8, 214, s, color, true, false, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 80 );
		return;
	}
	...
	if ( !( cg.snap->ps.eFlags & EF_VOTED ) ) {
		...
		CG_DrawStringExt( 8, 200, s, color, true, false, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 60 );

		s = va( CG_TranslateString( "YES(%s):%i, NO(%s):%i" ), str1, cgs.voteYes, str2, cgs.voteNo );
		CG_DrawStringExt( 8, 214, s, color, true, false, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 60 );
	} else {
		s = va( CG_TranslateString( "Y:%i, N:%i" ), cgs.voteYes, cgs.voteNo );
		CG_DrawStringExt( 8, 214, s, color, true, false, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 20 );
	}
}
...
static bool CG_DrawFollow( void ) {
	...
	if ( cg.snap->ps.pm_flags & PMF_LIMBO ) {
		...
		CG_DrawStringExt( INFOTEXT_STARTX, 68, deploytime, color, true, false, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 80 );
		...
		if ( cg.snap->ps.clientNum != cg.clientNum ) {
			...
			CG_DrawStringExt( INFOTEXT_STARTX, 86, deploytime, color, true, false, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 80 );
		}
	} else {
		// jpw
		...
		CG_DrawStringExt( 120, 68, name, color, true, true, BIGCHAR_WIDTH, BIGCHAR_HEIGHT, 0 );
	} // JPW NERVE
	return true;
}
...
static void CG_DrawWarmup( void ) {
	...
	if ( !sec ) {
		if ( cgs.gamestate == GS_WAITING_FOR_PLAYERS ) {
			...
			w = CG_DrawStrlen( s );
			CG_DrawStringExt( 320 - w * 6, 120, s, colorWhite, false, true, 12, 18, 0 );
...
//			CG_DrawStringExt( xx, y, va( "%i", i+1 ), hcolor, true, false, SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0 ); // JPW NERVE pulled per id request
