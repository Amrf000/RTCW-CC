...
int CG_DrawField( int x, int y, int width, int value, int charWidth, int charHeight, bool dodrawpic, bool leftAlign );      // NERVE - SMF
...
//		case WP_DYNAMITE: // JPW NERVE these recharge
	...
	if ( ammovalue > -1 && type == 0 ) {
		if ( shader ) {
			...
			// draw the clip count
			CG_DrawField( rect->x, rect->y, 4, ammovalue, 20 * scale, 32 * scale, true, false );
		}
	...
	if ( clipvalue > -1 && type == 1 ) {
		if ( shader ) {
			...
			// draw the clip count
			CG_DrawField( rect->x - startx, rect->y, 4, clipvalue, 20 * scale, 32 * scale, true, false );
		}
	}
}
...
static void CG_DrawHoldableItem( rectDef_t *rect, float scale, bool draw2D ) {
/*
	int		value;
	gitem_t	*item;
...
static void CG_DrawPlayerHealth( rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	if ( shader ) {
		...
		trap_R_SetColor( color );
		CG_DrawField( rect->x, rect->y, 3, value, 20 * scale, 32 * scale, true, true );           // NERVE - SMF
		...
		if ( ps->persistant[PERS_RESPAWNS_LEFT] >= 0 ) {
			...
			trap_R_SetColor( color );
			CG_DrawField( rect->x + 144, rect->y, 3, ps->persistant[PERS_RESPAWNS_LEFT], 20 * scale, 32 * scale, true, true );
		}
