...
static void CG_FlamethrowerFlame( centity_t *cent, vec3_t origin ) {
	...
	CG_FireFlameChunks( cent, origin, cent->lerpAngles, 1.0, true );
	return;
}
// done.
...
// JPW NERVE no tesla in multiplayer
	...
	for (i=0; i<numPoints; i++) {
		...
		if (VectorDistance( tagPos, cent->pe.teslaEndPoints[i] ) <= maxDist) {
			CG_DynamicLightningBolt( cgs.media.lightningBoltShader, tagPos, cent->pe.teslaEndPoints[i], 1+((cg.time%((i+2)*(i+3)))+i)%2, 20 + (float)(i%3)*5 + 6.0*random(), true, 1.0, 0, i*i*3 );
...
// JPW NERVE
				...
				if ( ps ) {
					if ( ps->ammoclip[weaponNum] ) {
						CG_FireFlameChunks( cent, flash.origin, angles, 1.0, false );
					}
				} else {
					CG_FireFlameChunks( cent, flash.origin, angles, 1.0, false );
				}
...
======================
*/
void CG_Bullet( vec3_t end, int sourceEntityNum, vec3_t normal, bool flesh, int fleshEntityNum, bool wolfkick, int otherEntNum2, int seed ) {
