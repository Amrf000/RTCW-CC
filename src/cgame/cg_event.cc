...
// jpw
	...
	case EV_MG42BULLET_HIT_WALL:
		...
		ByteToDir( es->eventParm, dir );
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, false, ENTITYNUM_WORLD, false, es->otherEntityNum2, es->effect1Time );
		break;

	case EV_MG42BULLET_HIT_FLESH:
		DEBUGNAME( "EV_MG42BULLET_HIT_FLESH" );
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, true, es->eventParm, false, es->otherEntityNum2, es->effect1Time );
		break;

	case EV_BULLET_HIT_WALL:
		DEBUGNAME( "EV_BULLET_HIT_WALL" );
		ByteToDir( es->eventParm, dir );
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, false, ENTITYNUM_WORLD, false, es->otherEntityNum2, 0 );
//		CG_MachineGunEjectBrass(cent); // JPW NERVE
		break;

	case EV_BULLET_HIT_FLESH:
		DEBUGNAME( "EV_BULLET_HIT_FLESH" );
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, true, es->eventParm, false, es->otherEntityNum2, 0 );
		break;

	case EV_WOLFKICK_HIT_WALL:
		DEBUGNAME( "EV_WOLFKICK_HIT_WALL" );
		ByteToDir( es->eventParm, dir );
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, false, ENTITYNUM_WORLD, true, es->otherEntityNum2, 0 );
		trap_S_StartSound( NULL, es->number, CHAN_AUTO, cgs.media.fkickwall );
		break;

	case EV_WOLFKICK_HIT_FLESH:
		DEBUGNAME( "EV_WOLFKICK_HIT_FLESH" );
		CG_Bullet( es->pos.trBase, es->otherEntityNum, dir, true, es->eventParm, true, es->otherEntityNum2, 0 );
...
//				CG_FireFlameChunks( cent, cent->currentState.origin, cent->lerpAngles, 0.1, false );
//				CG_FireFlameChunks( cent, cent->currentState.origin, cent->currentState.apos.trBase, 0.1, false );
//			}
//			else
