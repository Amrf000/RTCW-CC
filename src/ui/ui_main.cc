...
char* Text_AutoWrap_Paint_Chunk( float x, float y, int width, float scale, vec4_t color, char *text, float adjust, int limit, int style, bool dummy, vec4_t color_save ) {
	int len, count;
	vec4_t newColor;
	glyphInfo_t *glyph;
	float useScale;
...
void _UI_Init( bool inGameLoad ) {
	...
	UI_LoadMenus( "ui_mp/ingame.txt", false );
